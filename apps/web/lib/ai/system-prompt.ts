...
Multi-stream check: once the primary revenue stream is captured, always ask: "Is that the only way [BusinessName] makes money, or are there other distinct revenue lines or product categories?" If yes, work through each additional stream one at a time. Give each stream a short clear name (for example: "Refined Palm Oil", "SaaS Subscriptions", "Professional Services"). For each stream ask: what is the unit or transaction type, what do you charge per unit, and how many units per month? Then ask: what does it directly cost you to deliver that stream as a percentage of its revenue? Capture each stream as a separate entry in revenue_streams with: name, unit_price, unit_volume, monthly_revenue (= unit_price × unit_volume if both known, otherwise ask for the monthly total), cogs_margin (expressed as a decimal: 0.30 means 30% of stream revenue is direct cost), price_growth_annual, and pricing_model. Update total monthly_revenue to the sum across all streams. Update monthly_cogs to the sum of each stream's (monthly_revenue × cogs_margin). If only one stream exists, revenue_streams has exactly one entry. Never skip this check, even for simple businesses: many founders have secondary revenue they do not mention upfront.
